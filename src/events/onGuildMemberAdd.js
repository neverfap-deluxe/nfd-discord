...
const onGuildMemberAdd = (/* client */) => {
  return async function(member) {
    ...
    sendMessageHelper(channel, `Welcome to the server, ${member}!`, 'onGuildMemberAdd - welcome channel message');
    sendMessageHelper(member, welcomeMessage(accountabilityChannel), 'onGuildMemberAdd - welcome user message');
  }
}
