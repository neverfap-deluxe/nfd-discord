import { ClientUser, TextChannel, User, Client, Message } from 'discord.js';
...
In particular, how is your meditation going? If you've read the rules for <#${accountabilityChannel}>, you will have noticed that meditation is mandatory, and for very good reason! If you haven't yet had the chance, please type and enter \`!accountabilityChannel\` to learn the rules.\n
...
https://neverfapdeluxe.com/
`;
const commitmentTallyMessage3 = (accountabilityChannel: TextChannel): string =>
`Look at you go! You're absolute <#${accountabilityChannel}> pro now! (I mean, obviously not as good as my dear self *ahem*)\n
...
`;
const commitmentTallyMessage4 = (accountabilityChannel: TextChannel): string =>
`Oh wow! Another post into <#${accountabilityChannel}>! I may have to give you some sort of award for being so awe-inspiring!\n
...
`;
const commitmentTallyMessage5 = (accountabilityChannel: TextChannel): string =>
`Woah! Another delicious entry into the zany world of <#${accountabilityChannel}>! I honestly didn't know that you had it in you to keep going on this journey like an absolute BOSS (although what does a silly bot like me know?!)\n
...
`;
const commitmentTallyMessage6 = (_accountabilityChannel: TextChannel): string =>
...
`;
const commitmentTallyMessage7 = (_accountabilityChannel: TextChannel): string =>
...
`;
const commitmentTallyMessage10 = (_accountabilityChannel: TextChannel): string =>
...
`;
const commitmentTallyMessage14 = (_accountabilityChannel: TextChannel): string =>
...
`;
const commitmentTallyMessage21 = (_accountabilityChannel: TextChannel): string =>
...
`;
const commitmentTallyMessage28 = (accountabilityChannel: TextChannel): string =>
...
Is it weird to think that you've posted a month's worth of <#${accountabilityChannel}> posts? Well, it shouldn't because although it takes around 90 days to overcome porn addiction, even after you've overcome your porn addiction you will still need to continue meditating daily and working on your mental health.\n
...
`;
const commitmentTallyMessage35 = (_accountabilityChannel: TextChannel): string =>
...
`;
const commitmentTallyMessage42 = (_accountabilityChannel: TextChannel): string =>
...
You suck.
`;
const commitmentTallyMessage49 = (_accountabilityChannel: TextChannel): string =>
`Lucky 7.\n
...
`;
const commitmentTallyMessage50 = (_accountabilityChannel: TextChannel): string =>
`You did it.\n
...
const sendAutomatedMessage = async (count: number, discordUser: User, accountabilityChannel: TextChannel): Promise<Message> => {
  switch(count) {
    case 1: {
...
const automatedCommitmentTally = async (client: Client, dbUser: DBUser, discordUser: User) => {
  try {
    // TODO API Doesn't allow me to do this anymore?
    const accountabilityChannel: TextChannel = await getChannel(client, NFDChannelType.Accountability_Accountability);
    const accountabilityMessageCount: number = await knex('accountability_messages').where('db_users_id', dbUser.id).count();

    const count: number = parseInt(accountabilityMessageCount[0].count as string);

    const msg: Message = await sendAutomatedMessage(count, discordUser, accountabilityChannel);
...
export default automatedCommitmentTally;
