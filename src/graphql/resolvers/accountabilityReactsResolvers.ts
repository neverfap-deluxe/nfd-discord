...
export const accountabilityReactsQuery = {
  getAccountabilityReactsStats: async (/* _, {} */) => {
    const today = moment();
    const yesterday = moment().subtract(1, 'day');

    const accountabilityMessageCount = await knex('accountability_reacts').count();
    const accountabilityMessage24HourCount = await knex('accountability_reacts').whereBetween('created_at', [yesterday.toDate(), today.toDate()]).count();

    return {
      total: parseInt(accountabilityMessageCount[0].count as string),
      total24Hour: parseInt(accountabilityMessage24HourCount[0].count as string),
    }
  },
  ...
  getSomeFullAccountabilityReacts: async (_, { limit }) => {
    ...
    let full_accountability_reacts: any = [];
  ...
  getAccountabilityReact: async (_, { id }) => {
    const accountability_react: any = await knex('accountability_reacts')
      .where('id', id).select('*');
...
export const accountabilityReactsMutation = {
