...
export const accountabilityMessagesQuery = {
  getAccountabilityMessagesStats: async (/* _, {} */) => {
    const today = moment();
    const yesterday = moment().subtract(1, 'day');

    const accountabilityMessageCount = await knex('accountability_messages').count();
    const accountabilityMessage24HourCount = await knex('accountability_messages').whereBetween('created_at', [yesterday.toDate(), today.toDate()]).count();
...
export const accountabilityMessagesMutation = {
